...
A list of trusted IPs/subnets (IPv4 & IPv6) which will be allowed to access Emby.
...
Example:
```
emby_backup_dir: /local/backups/emby
```

Default:
```
emby_backup_dir: /usr/backups/emby
```
---
...
The name used for the Emby certbot certificate.

Example:
```
emby_cert_name: 'example.local'
```

Default:
```
emby_cert_name: '{{ emby_hostnames | first }}'
```
---
...
URL for the .deb file used to install Emby.

Example:
```
emby_deb_url: https://github.com/MediaBrowser/Emby.Releases/releases/download/4.6.6.0/emby-server-deb_4.6.6.0_amd64.deb
```

Default:
```
emby_deb_url: https://github.com/MediaBrowser/Emby.Releases/releases/download/4.6.7.0/emby-server-deb_4.6.7.0_amd64.deb
```
---
...
When true, ansible will ensure the folder exists for the backups generated by the emby plugin "Server Configuration Backup" and fetch a copy when the backups tasks are run.

Example:
```
emby_plugin_backups: true
```

Default:
```
emby_plugin_backups: false
```
---
...
## local_dns_zones
Roles: emby, kodi, pihole, transmission
...
## unifi_network_cert_name
Roles: unifi_network

The name of the certbot certificate to use for unifi_network role.

Example:
```
unifi_network_cert_name: 'example.local'
```

Default:
```
unifi_network_cert_name: '{{ default_domain }}'
```
---
...
A list of all the domains to include with the certbot certificate for unifi_network.
...
## unifi_video_cert_name
Roles: unifi_video

The name of the certbot certificate to use for the unifi_video role.

Example:
```
unifi_video_cert_name: example.local
```

Default:
```
unifi_video_cert_name: '{{ default_domain }}'
```
---
...
A list of all the domains to include with the certbot certificate for unifi_video.
...
The path to the recordings folder for unifi_video.

Example:
```
unifi_video_path: /mnt/videos
```

Default:
```
unifi_video_path: /media/recordings
```
---
